QUnit.extend(QUnit.assert, {
  ...
  deepClose: function(actual, expected, maxDifference, message) {    
      QUnit.push(goDeeper(actual, expected, maxDifference), actual, expected, message);
  }
});

function goDeeper(actual, expected, maxDifference) {
    ...
    if(l !== expected.length)
        return false;
    for(var i = 0; i < l; i++) {
        if(!goDeeper(actual[i], expected[i], maxDifference))
            return false;
    }
    return true;
}